...
complete -c $progname -n "$sync" -xa "$listall $listgroups (__paru_search_aur)"
...
# AUR search function for sync completions
function __paru_search_aur
    set -l token (commandline -ct)
    if test (string length -- "$token") -ge 3
        paru -Ssa -- "$token" 2>/dev/null | grep -E "^(aur)/" | awk -F'/' '{split($2, a, " "); printf "%s\t[%s]\n", a[1], $1}'
    end
end
